...
на каждый такт указывает на каждого из претендентов. Тот, на ком считалка
...
из 5 человек. Считалка разбита на такты символом |, а над каждым тактом
...
# Считать начинает претендент, следующий за выбывшим, его номер - 2,
он начинает с себя:
...
Задачу можно решить в цикле или рекурсивно — любой из этих способов подойдёт.
Если же вы сможете выполнить задание и тем и другим способом,
...
#  Через цикл 60 мс - принято!

# def count_it(applicants, beats):
#     persons = list(range(1, applicants + 1))
#     count = 0
#     idx = 0
#     while len(persons) > 1:
#         count += 1
...
#         idx = (idx + 1) % len(persons)
#     return persons[0]

#     # print(s)

#     # while len(applicants)

#     # count = 1
#     # if num == 0 or num == 1:
#     #     return count
#     # else:
#     #     return count_it(num - 1) + count_it(num - 2)


# if __name__ == '__main__':
#     applicants = int(input())
#     beats = int(input())
#     print(count_it(applicants, beats))

#     assert count_it(5, 2) == 3
#     assert count_it(5, 1) == 5
#     assert count_it(5, 6) == 4

#  Через рукурсию 35 мс - принято!


def count_it(applicants, beats, idx=0):
    """Функция реализации считалки."""
    if len(applicants) > 1:
        remove_idx = (idx + beats - 1) % len(applicants)
        idx = remove_idx % len(applicants)
        applicants.pop(remove_idx)
        return count_it(applicants, beats, idx)
    else:
        return applicants[0]


if __name__ == '__main__':
    applicants = list(range(1, int(input()) + 1))
    beats = int(input())
    print(count_it(applicants, beats))

    # assert count_it(list(range(1, 5 + 1)), 2) == 3
    # assert count_it(list(range(1, 5 + 1)), 1) == 5
    # assert count_it(list(range(1, 5 + 1)), 6) == 4
